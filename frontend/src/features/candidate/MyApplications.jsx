...
const MyApplications = () => {
    ...
    const filtered = useMemo(() => filter === "all" ? applications : applications.filter((item) => item.status === filter), [applications, filter]);
    ...
    return (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      {isLoading ? (<div className="flex min-h-[300px] items-center justify-center rounded-xl border border-border"><Loader2 className="mr-2 h-5 w-5 animate-spin text-primary"/> Loading applications...</div>) : isError ? (<EmptyState icon={AlertCircle} title="Could not load applications" description={error instanceof Error ? error.message : "Please try again."} action={{ label: "Try again", onClick: () => void refetch() }}/>) : !filtered.length ? (<EmptyState icon={Briefcase} title="No applications found" description="No applications match this filter." action={{ label: "Browse Jobs", onClick: () => navigate("/jobs") }}/>) : (<div className="space-y-3">
          {filtered.map((application) => (<Card key={application._id} className="transition-shadow hover:shadow-md">
              <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {application.job?._id && application.status !== "withdrawn" && (<Button variant="outline" size="sm" onClick={() => void openChat(application._id)}><MessageSquare className="mr-1.5 h-4 w-4"/> Message Recruiter</Button>)}
                  {!["offer", "rejected", "withdrawn"].includes(application.status) && (<Button variant="outline" size="sm" className="text-destructive" onClick={() => withdraw.mutate(application._id)} disabled={withdraw.isPending}>
