...
import { deleteMessage, editMessage, getMessages, getOlderMessages, markConversationRead, uploadChatAttachment, } from "./messageApi";
...
const ChatPage = () => {
    ...
    useEffect(() => {
        ...
        const edit = (message) => setMessages((items) => items.map((item) => item._id === message._id ? { ...item, ...message } : item));
        const remove = ({ messageId }) => setMessages((items) => items.filter((item) => item._id !== messageId));
        const reaction = ({ messageId, reactions }) => setMessages((items) => items.map((item) => item._id === messageId ? { ...item, reactions } : item));
        const typingStart = ({ conversationId }) => {
            if (conversationId === selectedId)
                setTyping(true);
        };
        const typingStop = ({ conversationId }) => {
            if (conversationId === selectedId)
                setTyping(false);
        };
        const actionError = ({ message }) => toast({ title: "Message action failed", description: message, variant: "destructive" });
    ...
    const sendMessage = () => {
        if ((!input.trim() && !pendingAttachments.length) || !selectedId)
            return;
