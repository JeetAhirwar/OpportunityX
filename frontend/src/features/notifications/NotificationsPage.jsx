...
const NotificationsPage = () => {
    ...
    const load = async () => setNotifications(await api.get("/notifications"));
    useEffect(() => { void load(); }, []);
    ...
    const filtered = useMemo(() => {
        ...
        if (tab === "application")
            return notifications.filter((item) => item.title.toLowerCase().includes("application"));
        if (tab === "message")
            return notifications.filter((item) => item.title === "New Message");
    ...
    return (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value={tab} className="mt-4">
          {!filtered.length ? <EmptyState icon={BellOff} title="No notifications" description="You're all caught up! Check back later."/> : (<div className="space-y-2">
