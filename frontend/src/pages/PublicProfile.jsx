...
import { MapPin, GraduationCap, Building2, FolderOpen, Globe, Award, Download, ExternalLink, Github, Linkedin, ArrowLeft, User, } from "lucide-react";
...
const PublicProfile = () => {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const data = await api.get(`/public/profile/${username}`, { skipAuth: true });
                setProfile(data);
            }
            catch (requestError) {
    ...
    return (<div className="min-h-screen bg-background">
      <SEOHead title={`${profile.name} - OpportunityX`} description={profile.bio?.slice(0, 155) || `${profile.name}'s professional profile`} canonical={`https://opportunityx.com/profile/${username}`} jsonLd={{
            "@context": "https://schema.org",
            "@type": "Person",
            name: profile.name,
            jobTitle: profile.title,
            url: `https://opportunityx.com/profile/${username}`,
            sameAs: [socials.linkedin, socials.github, socials.portfolio].filter(Boolean),
        }}/>
      <Navbar />
            ...
            <CardContent className="relative px-6 pb-6">
              <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-end -mt-12">
                <div className="h-24 w-24 rounded-full bg-muted border-4 border-card flex items-center justify-center overflow-hidden shadow-lg">
                  {profile.photo ? (<img src={profile.photo} alt={profile.name} className="h-full w-full object-cover"/>) : (<User className="h-10 w-10 text-muted-foreground"/>)}
                </div>
                <div className="flex-1">
                  <h1 className="font-display text-2xl font-bold">{profile.name}</h1>
                  <p className="text-muted-foreground">{profile.title}</p>
                  {profile.location && (<p className="flex items-center gap-1 text-sm text-muted-foreground mt-1">
                      <MapPin className="h-3.5 w-3.5"/> {profile.location}
                    </p>)}
                </div>
                <div className="flex gap-2">
                  {profile.resumeUrl && (<Button variant="outline" size="sm" asChild>
                      <a href={profile.resumeUrl} target="_blank" rel="noopener noreferrer"><Download className="mr-2 h-4 w-4"/> Resume</a>
                    </Button>)}
                  {socials.linkedin && (<Button variant="outline" size="icon" className="h-9 w-9" asChild>
                      <a href={socials.linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn profile"><Linkedin className="h-4 w-4"/></a>
                    </Button>)}
                  {socials.github && (<Button variant="outline" size="icon" className="h-9 w-9" asChild>
                      <a href={socials.github} target="_blank" rel="noopener noreferrer" aria-label="GitHub profile"><Github className="h-4 w-4"/></a>
                    </Button>)}
                  {socials.portfolio && (<Button variant="outline" size="icon" className="h-9 w-9" asChild>
                      <a href={socials.portfolio} target="_blank" rel="noopener noreferrer" aria-label="Portfolio"><Globe className="h-4 w-4"/></a>
                    </Button>)}
                </div>
              </div>
            </CardContent>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {experience.length > 0 && (<Card>
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    {experience.map((exp, i) => (<div key={i} className="relative pl-6 border-l-2 border-border pb-4 last:pb-0">
            ...
            {education.length > 0 && (<Card>
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    {education.map((edu, i) => (<div key={i} className="relative pl-6 border-l-2 border-border pb-4 last:pb-0">
            ...
            {projects.length > 0 && (<Card>
                <CardContent className="p-6">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {projects.map((proj, i) => (<div key={i} className="rounded-lg border border-border p-4">
                        <div className="flex items-center justify-between">
                          <h3 className="font-semibold">{proj.name}</h3>
                          {proj.url && (<a href={proj.url} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80">
            ...
            {certifications.length > 0 && (<Card>
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    {certifications.map((cert, i) => (<div key={i} className="rounded-lg bg-muted p-3">
