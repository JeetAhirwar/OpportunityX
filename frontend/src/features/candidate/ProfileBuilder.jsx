...
import { User, GraduationCap, Building2, FolderOpen, Globe, Plus, X, Save, Code2, Camera, Award, Briefcase, Loader2, } from "lucide-react";
...
const ProfileBuilder = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const data = normalizeProfile(await api.get("/candidate/profile"));
                setProfile(data);
    ...
    const validateProfile = () => {
        ...
        if (!phoneRegex.test(profile.phone.replace(/\D/g, "")))
            e.phone = "Invalid 10-digit number";
        if (!profile.location.trim())
            e.location = "Location required";
        if (!profile.title.trim())
            e.title = "Professional title required";
        if (profile.bio.trim().length < 20)
            e.bio = "Bio minimum 20 characters";
        ...
        if (profile.candidateType === "experienced") {
            profile.experience.forEach((exp, i) => {
                if (!exp.company)
                    e[`exp_company_${i}`] = "Company required";
                if (!exp.role)
                    e[`exp_role_${i}`] = "Role required";
                if (!exp.duration)
                    e[`exp_duration_${i}`] = "Duration required";
            });
        }
        setErrors(e);
    ...
    const completeness = (() => {
        ...
        if (profile.candidateType === "fresher")
            score += 12; // auto-complete for freshers
        if (profile.projects.length > 0)
            score += 8;
        if (profile.certifications.length > 0)
            score += 4;
        if (profile.socials.linkedin || profile.socials.github)
            score += 4;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (payload.candidateType === "fresher")
                payload.experience = [];
    ...
    const FieldError = ({ field }) => errors[field] ? <p className="text-xs text-destructive mt-1">{errors[field]}</p> : null;
    if (loading) {
    ...
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <PageHeader title="Profile Builder" description="Complete your professional profile" action={{
            label: isEditing
                ? saving
                    ? "Saving..."
                    : "Save Profile"
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="h-20 w-20 rounded-full bg-muted flex items-center justify-center overflow-hidden border-2 border-border">
                {profile.photo ? (<img src={profile.photo} alt="Profile" className="h-full w-full object-cover"/>) : (<Camera className="h-6 w-6 text-muted-foreground"/>)}
              </div>
          ...
          <div>
            ...
            <Input placeholder="Full Name" value={profile.name} onChange={(e) => updateField("name", e.target.value)} disabled={!isEditing}/>
      ...
      <Card>
        ...
        <CardContent>
          <RadioGroup value={profile.candidateType} onValueChange={(v) => isEditing && updateField("candidateType", v)} className="flex gap-4">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {profile.skills.map((skill) => (<Badge key={skill} variant="secondary" className="gap-1">
                {skill}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {profile.education.map((edu, i) => (<div key={i} className="grid gap-3 rounded-lg border border-border p-4 sm:grid-cols-3">
      ...
      <Card className={profile.candidateType === "fresher" ? "opacity-60" : ""}>
        <CardHeader className="flex flex-row justify-between items-center">
          ...
          {profile.candidateType === "experienced" && (<Button variant="outline" size="sm" disabled={!isEditing} onClick={() => updateField("experience", [...profile.experience, { company: "", role: "", duration: "", description: "" }])}><Plus className="h-4 w-4 mr-1"/> Add</Button>)}
        </CardHeader>
        <CardContent className="space-y-4">
          {profile.candidateType === "fresher" ? (<div className="rounded-lg bg-muted p-4 text-center">
              ...
              {profile.experience.map((exp, i) => (<div key={i} className="space-y-3 rounded-lg border border-border p-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {profile.projects.map((proj, i) => (<div key={i} className="space-y-3 rounded-lg border border-border p-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {profile.certifications.map((cert, i) => (<div key={i} className="grid gap-3 rounded-lg border border-border p-4 sm:grid-cols-2">
