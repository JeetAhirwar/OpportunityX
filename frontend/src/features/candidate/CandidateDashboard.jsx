...
const DashboardHome = () => {
    ...
    return (<div className="space-y-6">
      ...
      {loading ? (<div className="flex min-h-[240px] items-center justify-center rounded-xl border border-border"><Loader2 className="mr-2 h-5 w-5 animate-spin text-primary"/> Loading your dashboard...</div>) : hasError ? (<Card><CardContent className="p-6 text-sm text-destructive">Some dashboard data could not be loaded. Refresh the page to try again.</CardContent></Card>) : (<>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {recentApplications.length === 0 ? (<p className="text-sm text-muted-foreground">No applications yet. Browse jobs to get started.</p>) : recentApplications.map((application) => (<div key={application._id} className="flex items-center justify-between rounded-lg bg-secondary/50 p-3">
                <div>
...
const CandidateDashboard = () => {
    ...
    return (<>
      ...
      {showProfilePopup && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
