...
import { getAiJobRecommendations } from "@/features/ai/aiApi";
...
const JobRecommendations = () => {
    ...
    {loading ? (<div className="flex min-h-[280px] items-center justify-center rounded-xl border border-border"><Loader2 className="mr-2 h-5 w-5 animate-spin"/> Loading AI recommendations...</div>) : error ? (<EmptyState icon={AlertCircle} title="AI recommendations unavailable" description={error} action={{ label: "Complete profile", onClick: () => { window.location.href = "/candidate/profile"; } }}/>) : !recommendations.length ? (<EmptyState icon={Brain} title="No recommendations yet" description="Complete your profile skills and check back when active jobs are available."/>) : <div className="grid gap-4 lg:grid-cols-2">
      {recommendations.map((recommendation) => {
                ...
                return (<Card key={job._id} className="transition-shadow hover:shadow-md">
