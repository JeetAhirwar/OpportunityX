...
const SavedJobs = () => {
    ...
    return (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      {isLoading ? (<div className="flex min-h-[300px] items-center justify-center rounded-xl border border-border"><Loader2 className="mr-2 h-5 w-5 animate-spin text-primary"/> Loading saved jobs...</div>) : isError ? (<EmptyState icon={AlertCircle} title="Could not load saved jobs" description={error instanceof Error ? error.message : "Please try again."} action={{ label: "Try again", onClick: () => void refetch() }}/>) : jobs.length === 0 ? (<EmptyState icon={Bookmark} title="No saved jobs" description="Browse jobs and bookmark the ones you're interested in to find them here." action={{ label: "Browse Jobs", onClick: () => navigate("/jobs") }}/>) : (<div className="space-y-3">
          {jobs.map((job) => (<Card key={job._id} className="transition-shadow hover:shadow-md">
