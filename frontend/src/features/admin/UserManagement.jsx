...
import { createAdminUser, getAdminUsers, unwrapAdminData } from "@/features/admin/adminApi";
...
const UserManagement = () => {
    ...
    const updateStatus = async (item) => {
        try {
            const updated = unwrapAdminData(await api.patch(`/admin/users/${item._id}/status`, { isActive: !item.isActive }));
    ...
    const changeRole = async (item, role) => {
        ...
        if (!confirmed)
            return;
        try {
            const updated = unwrapAdminData(await api.patch(`/admin/users/${item._id}/role`, {
                role,
    ...
    const deleteUser = async (item) => {
        const adminDelete = item.role === "admin"
            ...
            : "";
        if (!window.confirm(`Permanently delete ${item.name}?${adminDelete}`))
            return;
        try {
