...
const ApplicantManagement = () => {
    ...
    useEffect(() => {
        setLoading(true);
        setError("");
        api.get(jobId ? `/applications/job/${jobId}` : "/applications/recruiter")
    ...
    const filtered = useMemo(() => statusFilter === "all" ? applicants : applicants.filter((item) => item.status === statusFilter), [applicants, statusFilter]);
    const updateStatus = async (id, status) => {
        try {
            const updated = asRecord(await api.patch(`/applications/${id}/status`, { status }));
    ...
    return (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      {loading ? (<div className="flex min-h-[300px] items-center justify-center rounded-xl border border-border">
          <Loader2 className="mr-2 h-5 w-5 animate-spin text-primary"/>
          <span className="text-sm text-muted-foreground">Loading applicants...</span>
        </div>) : error ? (<EmptyState icon={AlertCircle} title="Could not load applicants" description={error} action={{ label: "Try again", onClick: () => setReloadKey((key) => key + 1) }}/>) : !filtered.length ? (<EmptyState icon={User} title="No applicants found" description="Applicants for your jobs will appear here."/>) : (<div className="space-y-3">
          {filtered.map((applicant) => (<Card key={applicant._id} className="transition-shadow hover:shadow-md">
