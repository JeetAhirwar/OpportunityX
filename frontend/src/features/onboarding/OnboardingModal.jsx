...
import { User, FileText, Briefcase, CheckCircle, ChevronRight, ChevronLeft, MapPin, Upload, Plus, X, Sparkles, } from "lucide-react";
...
const OnboardingModal = ({ open, onComplete }) => {
    ...
    const isProfileStepValid = profile.headline.trim().length > 0 &&
      ...
      <DialogContent className="max-w-xl gap-0 overflow-hidden p-0 sm:rounded-2xl [&>button]:hidden">
        ...
        <div className="px-6 pt-6">
          ...
          <div className="mt-4 flex items-center justify-between">
            {steps.map((s) => (<div key={s.id} className="flex flex-col items-center gap-1">
                <div className={cn("flex h-9 w-9 items-center justify-center rounded-full border-2 transition-all", step > s.id
          ...
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
              ...
              {step === 0 && (<div className="space-y-5">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input placeholder="e.g. Senior React Developer" value={profile.headline} onChange={(e) => setProfile((current) => ({ ...current, headline: e.target.value }))}/>
                    </div>
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input className="pl-9" placeholder="City, Country" value={profile.location} onChange={(e) => setProfile((current) => ({ ...current, location: e.target.value }))}/>
                        </div>
                      </div>
                      <div className="space-y-2">
                        <Label>Phone *</Label>
                        <Input placeholder="+1 234 567 890" value={profile.phone} onChange={(e) => setProfile((current) => ({ ...current, phone: e.target.value }))}/>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label>Skills</Label>
                      ...
                      {skills.length > 0 && (<div className="flex flex-wrap gap-1.5 pt-1">
                          {skills.map((s) => (<Badge key={s} variant="secondary" className="gap-1 pr-1">
                              {s}
              ...
              {step === 1 && (<div className="space-y-5">
                  ...
                  <div className={cn("flex flex-col items-center justify-center rounded-xl border-2 border-dashed p-10 text-center transition-colors cursor-pointer", resumeFile ? "border-primary bg-primary/5" : "border-border hover:border-primary/50")} onClick={() => document.getElementById("onboarding-resume")?.click()}>
                    {resumeFile ? (<>
                        ...
                        <Button variant="ghost" size="sm" className="mt-3 text-destructive" onClick={(e) => { e.stopPropagation(); setResumeFile(null); }}>
                          Remove
                        </Button>
                      </>) : (<>
                  ...
                  <input id="onboarding-resume" type="file" accept=".pdf,application/pdf" className="hidden" onChange={(e) => selectResume(e.target.files?.[0])}/>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {JOB_TYPES.map((t) => (<Badge key={t} variant={selectedTypes.includes(t) ? "default" : "outline"} className="cursor-pointer select-none transition-colors" onClick={() => toggleSelection(t, selectedTypes, setSelectedTypes)}>
                            {t}
                          </Badge>))}
                      </div>
                    </div>
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {WORK_MODES.map((m) => (<Badge key={m} variant={selectedModes.includes(m) ? "default" : "outline"} className="cursor-pointer select-none transition-colors" onClick={() => toggleSelection(m, selectedModes, setSelectedModes)}>
                            {m}
                          </Badge>))}
                      </div>
                    </div>
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {INDUSTRIES.map((i) => (<Badge key={i} variant={selectedIndustries.includes(i) ? "default" : "outline"} className="cursor-pointer select-none transition-colors" onClick={() => toggleSelection(i, selectedIndustries, setSelectedIndustries)}>
                            {i}
                          </Badge>))}
                      </div>
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input type="number" placeholder="e.g. 80000" value={salaryMin} onChange={(e) => setSalaryMin(e.target.value)}/>
                    </div>
                  </div>
                </div>)}

              {/* Step 3 — Complete */}
              {step === 3 && (<div className="flex flex-col items-center justify-center space-y-4 py-8 text-center">
        ...
        <div className="flex items-center justify-between border-t border-border bg-secondary/30 px-6 py-4">
          {step > 0 && step < 3 ? (<Button variant="ghost" size="sm" onClick={() => setStep(step - 1)}>
              <ChevronLeft className="mr-1 h-4 w-4"/> Back
            </Button>) : (<div />)}
          {step < 3 ? (<div className="flex gap-2">
              {step === 1 && (<Button variant="ghost" size="sm" onClick={() => setStep(step + 1)}>
                  Skip
                </Button>)}
