...
import { Search, MapPin, ArrowRight, Briefcase, Building2, CheckCircle2, Upload, Brain, MessageSquare, Zap, BookmarkPlus, ChevronRight, ShieldCheck, ClipboardCheck, Sparkles, Bot, Lock, BarChart3, Radio, FileSearch, } from "lucide-react";
...
const initials = (value) => (value?.trim()?.slice(0, 2) || "OX").toUpperCase();
...
const Index = () => {
    ...
    const [metrics, setMetrics] = useState({ activeJobs: null, featuredJobs: 0 });
    useEffect(() => {
        const fetchFeaturedJobs = async () => {
            try {
                const [featuredResponse, jobsResponse] = await Promise.all([
                    api.get("/jobs/featured", { skipAuth: true }),
                    api.get("/jobs?limit=1", { skipAuth: true }),
                ]);
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden border-b border-border/80 bg-card py-14 md:py-20">
              ...
              <motion.div variants={fadeInUp} custom={3} className="max-w-3xl">
                <div className="flex flex-col gap-3 rounded-lg border border-border bg-background p-3 shadow-sm md:flex-row md:items-center">
                  ...
                  <div className="relative flex-1">
                    ...
                    <Input placeholder="City, state, or remote" value={searchLocation} onChange={(e) => setSearchLocation(e.target.value)} className="border-0 bg-transparent pl-10 shadow-none focus-visible:ring-0"/>
                  </div>
            ...
            { label: "Recruiter chat", value: "Application scoped", icon: Radio },
        ].map((item) => (<div key={item.label} className="rounded-lg border border-border/70 bg-background/50 p-4">
                  ...
                  <div className="rounded-lg border border-primary/30 bg-primary/10 p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {["Candidate", "Recruiter", "Admin"].map((label) => (<div key={label} className="rounded-md bg-background/60 px-3 py-2 text-center text-xs font-semibold">
                          {label}
                        </div>))}
                    </div>
                  </div>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          ...
          {loadingFeatured ? (<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: 3 }).map((_, index) => (<div key={index} className="h-64 animate-pulse rounded-lg border border-border/70 bg-card/60"/>))}
            </div>) : featuredJobs.length === 0 ? (<div className="rounded-lg border border-border/70 bg-card/70 p-10 text-center">
                  ...
                  <Card className="group h-full transition-colors hover:border-primary/35">
                    <CardContent className="flex h-full flex-col p-5">
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {job.workMode && (<span className="rounded-full bg-success/10 px-2.5 py-1 font-semibold text-success">
                            {job.workMode === "onsite" ? "On-site" : job.workMode}
                          </span>)}
                        {job.experienceLevel && (<span className="rounded-full bg-primary/10 px-2.5 py-1 font-semibold text-primary">
                            {job.experienceLevel}
                          </span>)}
                      </div>

                      {!!job.skills?.length && (<div className="mt-4 flex flex-wrap gap-2">
                          {job.skills.slice(0, 4).map((skill) => (<span key={skill} className="rounded-full border border-border/70 px-2.5 py-1 text-xs text-muted-foreground">{skill}</span>))}
                        </div>)}
      ...
      <section className="border-y border-border/80 bg-card py-20">
        <div className="container mx-auto px-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {productStrengths.map((item, index) => (<motion.div key={item.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.04 }}>
            ...
            { title: "Recruiter Flow", items: recruiterFlow },
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    {group.items.map((step, index) => (<div key={step.title} className="flex gap-4">
            ...
            <div className="grid items-center md:grid-cols-2">
              <div className="p-8 md:p-12">
                ...
                <ul className="mb-8 space-y-3">
                  {["Resume analysis with ATS guidance", "Candidate match scoring for recruiters", "Career assistant prompts", "Provider-safe unavailable states"].map((item) => (<li key={item} className="flex items-center gap-2 text-sm">
      ...
      <section className="py-20">
        ...
        ].map((item) => (<Card key={item.title} className="h-full">
                <CardContent className="p-6">
                  <h3 className="font-display text-lg font-semibold">{item.title}</h3>
