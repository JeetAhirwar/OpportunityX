...
import { getAdminRecruiters, unwrapAdminData } from "@/features/admin/adminApi";
...
const RecruiterApproval = () => {
    ...
    const approve = async (company) => {
        if (!window.confirm(`Approve ${company.companyName || company.recruiterName}?`))
            return;
        try {
            const updated = unwrapAdminData(await api.patch(`/admin/recruiters/${company._id}/approve`));
    ...
    const reject = async (company) => {
        const reason = window.prompt("Enter the rejection reason:");
        if (!reason?.trim())
            return;
        try {
            const updated = unwrapAdminData(await api.patch(`/admin/recruiters/${company._id}/reject`, { reason }));
            setCompanies((items) => items.map((item) => item._id === company._id ? { ...item, ...updated } : item));
      ...
      {loading ? <div className="flex min-h-[300px] items-center justify-center"><Loader2 className="mr-2 h-5 w-5 animate-spin"/> Loading recruiter profiles...</div>
            : error ? <EmptyState icon={AlertCircle} title="Could not load approvals" description={error}/>
                : !companies.length ? <EmptyState icon={Users} title="No recruiter profiles" description="Recruiter verification submissions will appear here."/>
                    : <div className="space-y-3">{companies.map((company) => (<Card key={company._id}><CardContent className="p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
