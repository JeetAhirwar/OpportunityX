...
const generate = async ({ systemPrompt, userPrompt, temperature = 0.4, maxTokens = 800, jsonMode = false }) => {
  if (!configured()) throw new ProviderError("OpenAI API key is not configured", "missing_key", 503);
  ...
  if (!text) throw new ProviderError("OpenAI returned an empty response", "malformed_response", 502);
