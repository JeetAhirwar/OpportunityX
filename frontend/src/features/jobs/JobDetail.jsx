...
import { MapPin, DollarSign, Clock, Building2, Briefcase, BookmarkPlus, Bookmark, Share2, ArrowLeft, CheckCircle2, Users, Calendar, Mail, } from "lucide-react";
...
const companyInitials = (company) => (company?.trim()?.slice(0, 2) || "OX").toUpperCase();
const formatWorkMode = (workMode) => workMode === "onsite" ? "On-site" : workMode ? `${workMode.charAt(0).toUpperCase()}${workMode.slice(1)}` : "Work mode TBD";
const safeDateDistance = (date) => {
    if (!date)
...
const JobDetail = () => {
    ...
    const saved = useMemo(() => Boolean(id && savedJobs.some((savedJob) => savedJob._id === id)), [id, savedJobs]);
    useEffect(() => {
    ...
    const openApplication = () => {
        if (requireCandidate("apply to"))
            setDialogOpen(true);
    };
    const handleApply = async () => {
        if (!id || !job || !requireCandidate("apply to"))
            return;
        try {
    ...
    return (<div className="min-h-screen bg-background">
      <SEOHead title={`${job.title} at ${job.company} - OpportunityX`} description={`Apply for ${job.title} at ${job.company}. ${salary} - ${job.location}`} jsonLd={{
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        ].map((item) => (<div key={item.label} className="flex items-center gap-2 text-sm capitalize text-muted-foreground"><item.icon className="h-4 w-4"/> {item.label}</div>))}
                  </div>

                  <div className="mt-6 flex flex-wrap gap-3">
                    {applied ? (<Button disabled className="bg-success text-success-foreground"><CheckCircle2 className="mr-2 h-4 w-4"/> Applied</Button>) : (<Button onClick={openApplication}>Apply Now</Button>)}
